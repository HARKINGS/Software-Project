  ...
  data.forEach((account, index) => {
    tbody.innerHTML += `
                <tr data-id="${account.id}">
                    ...
                    <td>
                        <button class="change-btn" onclick="editRow(${index})">Sửa</button>
...
function editRow(index) {
  ...
  row.innerHTML = `
        ...
        <td><input type="text" value="${account.residentId}" class="input edit-residentId non-interactive" /></td>
        <td>
...
function filterData() {
  ...
  const filteredData = accounts.filter((account) => {
    ...
    const residentIdMatch =
      userIDFilter === "" ||
      String(account.residentId).toLowerCase().includes(userIDFilter);
